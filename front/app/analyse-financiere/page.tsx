...
import { TrendingUp, FileText, DollarSign, BarChart3, Users, Calculator, Database, PieChart, Upload, Calendar, LayoutDashboard, LogOut, User, Sun, Moon, CheckSquare, Package, Cpu, FileSpreadsheet } from 'lucide-react';
import Notification, { useNotification } from '../components/Notification';
import HamburgerMenu from '../components/HamburgerMenu';
import ProfileAvatar from '../components/ProfileAvatar';

export default function AnalyseFinanciereHomePage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-blue-50 via-blue-100 to-blue-200 dark:from-[#0f0d1a] dark:via-[#1a1628] dark:to-[#1a0f2a] text-gray-800 dark:text-white p-8 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="mb-12 text-center">
          <h1 className="text-6xl font-extrabold mb-4 text-white">
            Analyse <span className="text-blue-600 dark:text-[#8c68d8]">Financière</span>
          </h1>
          <p className="text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
            Explorez et analysez vos données financières avec nos outils spécialisés
          </p>
        </div>

        {/* Grid de cartes style menu principal */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {/* Coûts salariaux - Style menu principal */}
          <Link href="/analyse-financiere/couts-salariaux" className="group h-full">
            <div className="border-2 border-gray-800 dark:border-white bg-blue-100/50 dark:bg-white/5 backdrop-blur-sm text-gray-800 dark:text-white font-semibold py-12 px-8 rounded-2xl shadow-xl text-xl hover:bg-blue-200/50 dark:hover:bg-white/10 hover:scale-105 transition-all duration-300 cursor-pointer flex flex-col items-center gap-6 group h-full min-h-[400px] justify-between">
              <div className="w-20 h-20 bg-gradient-to-br from-blue-600 to-blue-700 rounded-2xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-shadow">
                <TrendingUp className="w-10 h-10 text-white" />
              </div>
              <div className="text-center flex-1 flex flex-col justify-center">
                <h2 className="text-3xl font-bold mb-2">Coûts Salariaux</h2>
                <p className="text-lg text-gray-600 dark:text-gray-300 leading-relaxed">
                  Analysez en détail vos coûts salariaux avec des filtres avancés, 
                  des outils d'édition et des fonctionnalités d'import de données.
                </p>
              </div>
              <div className="flex items-center gap-3 text-gray-600 dark:text-gray-400 font-semibold group-hover:gap-4 transition-all">
                <span className="group-hover:underline">Ouvrir l'analyse</span>
                <span className="text-2xl group-hover:translate-x-1 transition-transform">→</span>
              </div>
            </div>
          </Link>

          {/* Analyse FEC - Style menu principal */}
          <Link href="/analyse-financiere/analyse-fec" className="group h-full">
            <div className="border-2 border-gray-800 dark:border-white bg-blue-100/50 dark:bg-white/5 backdrop-blur-sm text-gray-800 dark:text-white font-semibold py-12 px-8 rounded-2xl shadow-xl text-xl hover:bg-blue-200/50 dark:hover:bg-white/10 hover:scale-105 transition-all duration-300 cursor-pointer flex flex-col items-center gap-6 group h-full min-h-[400px] justify-between">
              <div className="w-20 h-20 bg-gradient-to-br from-orange-600 to-orange-700 rounded-2xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-shadow">
                <FileText className="w-10 h-10 text-white" />
              </div>
              <div className="text-center flex-1 flex flex-col justify-center">
                <h2 className="text-3xl font-bold mb-2">Analyse FEC</h2>
                <p className="text-lg text-gray-600 dark:text-gray-300 leading-relaxed">
                  Explorez et contrôlez vos écritures comptables avec des outils 
                  d'analyse avancés et des rapports détaillés.
                </p>
              </div>
              <div className="flex items-center gap-3 text-gray-600 dark:text-gray-400 font-semibold group-hover:gap-4 transition-all">
        ...
        <div className="mt-16 max-w-4xl mx-auto">
          <div className="border-2 border-gray-800 dark:border-white bg-blue-100/50 dark:bg-white/5 backdrop-blur-sm rounded-2xl p-8">
            <h3 className="text-2xl font-bold text-gray-800 dark:text-white mb-6 text-center">
              Outils d'analyse avancés
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="flex items-center gap-4 p-4 bg-white/20 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10">
                <div className="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 rounded-xl flex items-center justify-center">
                  <BarChart3 className="w-6 h-6 text-blue-600" />
                </div>
                <div>
                  <div className="font-semibold text-gray-800 dark:text-white">Analyses détaillées</div>
                  <div className="text-sm text-gray-600 dark:text-gray-400">Graphiques et statistiques</div>
                </div>
              </div>
              <div className="flex items-center gap-4 p-4 bg-white/20 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10">
                ...
                <div>
                  <div className="font-semibold text-gray-800 dark:text-white">Import de données</div>
                  <div className="text-sm text-gray-600 dark:text-gray-400">Excel, CSV, API</div>
                </div>
              </div>
              <div className="flex items-center gap-4 p-4 bg-white/20 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10">
                ...
                <div>
                  <div className="font-semibold text-gray-800 dark:text-white">Rapports</div>
                  <div className="text-sm text-gray-600 dark:text-gray-400">Export PDF/Excel</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <Link
        href="/analyse-financiere/fec-test"
        className="fixed bottom-6 right-6 z-30 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer"
      >
        Test
      </Link>
    </main>
